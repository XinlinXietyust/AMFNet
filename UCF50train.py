...
parser.add_argument('--dataset', default='part_1', type=str, help='dataset')
...
train_loader = data.DataLoader(train_dataset, batch_size=args.bs, shuffle=False)
...
for epoch in range(start_epoch, start_epoch + args.epoch):
    ...
    writer.add_scalar('loss/(train_loss + float("1e-8"))', loss_avg / (len(train_loader) + float("1e-8")), epoch)
    writer.add_scalar('loss/(train_loss_att + float("1e-8"))', loss_att_avg / (len(train_loader) + float("1e-8")), epoch)
    ...
    with torch.no_grad():
        ...
        mae /= len(test_loader) + float("1e-8")
        mse /= len(test_loader) + float("1e-8")
        mse = mse ** 0.5
